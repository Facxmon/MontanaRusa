...
run('TestsValidacion.m')      % quince tests, termina con error si alguno falla
```
...
`TestsValidacion.m` implementa **quince** tests y termina con error si alguno falla. Todos pasan por la API pública de los elementos, para que lo que se verifica sea el mismo camino que usa el usuario.

...
El modo pide en cada punto del arco el $+G_z$ que la curva límite admite para la duración del evento sostenido, medida con el reloj de cada nivel (abajo) y convertida a duración del prototipo multiplicando por $\sqrt\lambda_{loop}$ (fórmula de conversión de duración: [`memoria_de_calculo.md` §5.1](memoria_de_calculo.md#51-definiciones-normativas-aplicables)). Durante la rampa de entrada la duración es cero: el objetivo es el límite de evento corto, el mismo que el arco pide en su primer punto ([§4](#4-sub-tramos)). La curva que persigue **no es la tabla literal sino `LimiteDeDiseno`**: la misma tabla con los quiebres de pendiente redondeados por debajo en una ventana de `Parametros.SemianchoDeSuavizadoNormativo` (0.05 s de prototipo), para que el objetivo sea $C^1$ y $\kappa(s)$ no herede saltos de $d\kappa/ds$ que el roll helicoidal convertiría en escalones de $G_y$ ([`memoria_de_calculo.md` §7.8](memoria_de_calculo.md#78-rampas-de-curvatura-suaves-por-qué-la-clotoide-lineal-no-alcanza-con-roll-helicoidal)); queda 0.025 G por debajo de la Fig. 10 en 1.0 s y 0.007 G en 2.0 s, y con semiancho 0 es la tabla. La verificación usa siempre la tabla literal. Todo eso va dividido por `FactorDeSeguridadNormativo` (bloque 3 de `ParametrosPorDefecto`, 1.0 = sin margen) y lleva descontada `TolObjetivoDeG` (abajo): el diseño nace con margen y la verificación sigue siendo contra la norma literal. El chequeo posterior **"Gz objetivo del modo alcanzado"** lee ese mismo objetivo (`Track.ObjetivoNormativo`) y mide sobre la simulación cuánto se apartó la $G_z$ del pasajero en el arco (tolerancia `TolObjetivoDeG`); falla si el objetivo era inalcanzable a esa velocidad —la cuadrática del transporte inverso no tiene raíz real— y lo dice.

**El reloj de cada nivel.** La norma no mide "cuánto dura el arco": mide, para cada nivel $G^*$, cuánto dura el evento sostenido en que $G \geq G^*$, y ese evento empieza **en la clotoide de entrada**, cuando la G cruza el nivel, no cuando arranca el arco. Un reloj único desde el inicio del arco le regalaba a cada nivel el tiempo que la clotoide ya llevaba por encima de él, y `VerificarLimitesNormativos` lo cobraba: en el loop vertical de referencia (v₀ = 4.6 m/s, `RadioDelLoop` 0.11) el criterio "+Gz (Fig. 10)" fallaba por **0.36 G** al nivel 4.4 G, sobre la misma G que el modo había pedido. Arrancar el reloj único en la clotoide lo arregla, pero le quita a la meseta de 6 G toda la duración de la clotoide (0.5 s de 1.0 con los defaults): es pagar por todos los niveles lo que sólo deben los más bajos.

Lo que hace el modo (`ObjetivoNormativoPorNiveles`) es darle a cada nivel su propio reloj. Con $t_c(G^*)$ el primer instante en que la $G_z$ registrada antes del arco (acondicionamiento y clotoide) alcanza $G^*$ —o el inicio del arco si no lo alcanzó, o el primer nodo si ya entró por encima— y $L_d(D)$ la curva de diseño, el objetivo del arco es la curva paramétrica

$$t(D) = t_c\big(L_d(D)\big) + D/\sqrt{\lambda_{loop}}, \qquad G_{z,obj}(D) = L_d(D):$$

el objetivo pasa por el nivel $L_d(D)$ exactamente cuando ese nivel agota su duración admisible $D$, contada desde que se lo cruzó. Todo nivel queda dentro de la norma por construcción. Los niveles cercanos al máximo se cruzan al final de la clotoide, así que la meseta apenas se acorta (termina en 0.95 s de prototipo desde el arco, como antes); los de abajo se cruzaron antes y expiran antes: en el loop de referencia el descenso arranca en la misma meseta pero baja a −2.5 ÷ −3.4 G/s en vez de −2, y el arco termina en 3.85 G a los 3.9 s. La tabla queda en `Track.ObjetivoNormativo` (tiempo del modelo, paso uniforme, evaluada con una cúbica de Hermite por `EvaluarObjetivoNormativo`) y es exactamente lo que usan el criterio "Gz objetivo del modo alcanzado", los tests y la línea de trazos del gráfico de $G_z$. La duración de la curva de $G_y$ del dive loop sigue contando desde el inicio del arco: su arco dura menos de 1 s y la Fig. 8 es plana ahí.

**El margen `TolObjetivoDeG` se descuenta del objetivo.** Un objetivo puesto exactamente sobre la curva la viola con el ruido numérico: la $G_z$ máxima del loop era 6.0013 G contra 6.0 de límite, y el chequeo de cumplimiento es estricto. `TolObjetivoDeG` (0.05 G) es lo que el criterio "Gz objetivo del modo alcanzado" le tolera de error al transporte inverso; descontarlo del objetivo es lo que hace que ese criterio **garantice** el de "+Gz (Fig. 10)": si el pasajero recibió el objetivo dentro de la tolerancia, está dentro de la norma. Es la misma razón por la que el $G_y$ objetivo del dive loop ya lo descontaba (§5.1). Con los defaults el loop pone 5.95 G y el criterio "+Gz (Fig. 10)" pasa con −0.049 G de exceso; achicar `TolObjetivoDeG` acerca la meseta a 6.0 y achica la tolerancia del criterio en la misma medida.

Dos detalles de continuidad, porque el objetivo entra derivado en $\kappa(s)$ y $d\kappa/ds$ en el roll helicoidal (§7.8 de la memoria): (i) la clotoide llega a su máximo con pendiente nula, así que $t_c(G)$ tiene una raíz cuadrada centrada en lo que la clotoide **alcanzó**; si eso quedó por debajo del máximo de diseño (el error del transporte inverso, ~0.0004 G) las raíces de $t_c$ y del quiebre redondeado de $L_d$ quedan descentradas y el descenso arrancaría con un escalón. Se corre el registro hacia arriba hasta el máximo de diseño (cada nivel se da por cruzado 0.05 ms antes de lo que se cruzó: conservador; hasta 0.01 G, para no tapar un objetivo inalcanzable) en vez de bajar la curva, para que el arco arranque en el mismo objetivo con el que la clotoide terminó y $\kappa$ no salte en la frontera —un salto de 0.0004 G en el objetivo era un pico de 7 G/s de jerk lateral a 1 mm de paso—. (ii) $t_c(G)$ se interpola con pchip y la tabla se evalúa con la cúbica de Hermite: con interpolación lineal cada nodo de la clotoide era un salto de pendiente del objetivo, que el roll helicoidal convertía en un peine de jerk lateral dependiente del paso (test 15). Si $t(D)$ retrocediera en algún tramo (clotoide larga) gana el nivel más bajo y `Objetivo.HuboPliegue` lo anota; con los cuatro elementos y los defaults no pasa.
...
| 12 | El modo normativo pone en el pasajero el $+G_z$ objetivo que persiguió (`Track.ObjetivoNormativo`, reloj por nivel), y ese objetivo arranca en el límite de evento corto menos `TolObjetivoDeG` y nunca lo supera; en los cuatro elementos (over-banked turn a 240°: con 180° las rampas suaves le dejan 0.05 s de arco) | desvío máximo $\lvert G_z - G_{obj}(t)\rvert$ sobre el arco: 0.003 G (loop), 0.002 G (hélice), 0.000 G (over-banked turn, dive loop); un arco vacío cuenta como falla |

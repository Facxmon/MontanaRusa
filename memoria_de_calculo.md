...
**Diseñar con el mismo criterio: el reloj de cada nivel.** El modo `GNormativaMaxima` invierte esta verificación —pide en cada punto el $G_z$ que la curva admite— y tiene que medir la duración como la mide la norma: **por nivel, desde que la G cruza ese nivel**. La G cruza cada nivel en la clotoide de entrada, antes de que arranque el arco; un reloj único que arrancara en el arco le regalaría a cada nivel el tiempo que la clotoide ya llevaba por encima de él (en el loop de referencia, 0.36 G de exceso al nivel 4.4 G, sobre la misma G que el modo había pedido). Con $t_c(G^*)$ el instante en que la G registrada antes del arco cruzó $G^*$ y $L_d(D)$ la curva de diseño de §7.8, el objetivo del arco es la curva paramétrica $t(D) = t_c(L_d(D)) + D/\sqrt{\lambda_{loop}}$, $G_{z,obj} = L_d(D)$: cada nivel se abandona exactamente cuando agota su duración admisible contada desde su propio cruce. La meseta apenas se acorta (los niveles cercanos al máximo se cruzan al final de la clotoide) y el descenso es más rápido que la pendiente de la curva (los niveles bajos se cruzaron antes). Como el chequeo de cumplimiento es estricto y el objetivo se alcanza con un error numérico del orden de $10^{-3}$ G, el objetivo lleva además descontada `TolObjetivoDeG` (0.05 G): si el pasajero recibió el objetivo dentro de esa tolerancia, está dentro de la norma. Con los valores por defecto el loop vertical pone 5.95 G. Implementación y detalles de continuidad en [`documentacion_generador_elementos.md` §5](documentacion_generador_elementos.md#5-los-cuatro-modos-de-curvatura).
...
**El mismo problema aparece dentro del arco en el modo normativo, y se resuelve igual.** La curva límite de la Fig. 10 es lineal por tramos, así que el objetivo $G_z(t)$ tiene quiebres de pendiente en 1.0 y 2.0 s de prototipo (y en los demás puntos de la tabla); $\kappa(s)$ los hereda, $d\kappa/ds$ salta ahí y $\phi''$ también: un escalón de $G_y$ de $\sim 0.01$ G, un orden de magnitud menor que el de las rampas pero de la misma naturaleza. El objetivo de **diseño** es entonces `LimiteDeDiseno`: la tabla con cada quiebre redondeado **por debajo** en una ventana de $\pm w$ (`Parametros.SemianchoDeSuavizadoNormativo`, 0.05 s de prototipo), restándole a la magnitud una corrección no negativa que anula el salto de pendiente —$h = (|\Delta m|/4)(w-|t|)^2/w$ en los quiebres cóncavos, $h = (|\Delta m|/2w^2)(w-|t|)^2|t|$ en los convexos, con $h(\pm w) = h'(\pm w) = 0$—. El objetivo queda $C^1$ y nunca por encima de la norma (0.025 G por debajo en 1.0 s, 0.007 G en 2.0 s); la verificación sigue comparando contra la tabla literal. Con las dos medidas, el onset lateral del loop vertical que reporta la verificación vale 8.8 G/s a 2 mm de paso y 9.1 G/s a 1 mm: un jerk físico (el de $d^2\kappa/ds^2$ al arrancar la rampa de salida), no un artefacto. El test 15 fija las dos propiedades. El reloj por nivel de §5.7 se construye con la misma exigencia: el cruce de cada nivel se interpola con pchip y la tabla del objetivo se evalúa con una cúbica de Hermite, porque con interpolación lineal cada nodo de la clotoide era un salto de pendiente del objetivo y el onset lateral volvía a depender del paso.
